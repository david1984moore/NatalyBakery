...
**Convention:**
- **Mobile-only styles** = unprefixed classes and `sm:` only. When you **do not** touch these, mobile stays unchanged.
- **Desktop-only styles** = use only `md:`, `desktop:`, `lg:`, `xl:`, or `2xl:`. `md:` and `desktop:` use `(hover: hover) and (pointer: fine)` so rotated phones do not get desktop layout.
...
### Rules for "desktop-only" work (mobile untouched)

When the goal is **desktop changes only**:

1. **Only add or change** classes that use: `md:`, `lg:`, `xl:`, or `2xl:`, or `desktop:`
2. **Do not add or change**: unprefixed (base) classes or `sm:` classes — unless you explicitly intend to change mobile.

### Same component, different breakpoints

Many components use both base/`sm:` (mobile) and `md:`+ (desktop) in the same `className`:
...
### Completely different markup for mobile vs desktop
