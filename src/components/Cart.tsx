      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            {/* Cart wrapper — CSS handles all mobile positioning via .cart-mobile-wrapper */}
            <div className="cart-mobile-wrapper md:fixed md:inset-0 md:z-[999] md:flex md:items-center md:justify-center md:p-4 md:pointer-events-none">
              <div
                className="cart-mobile-panel md:w-full md:max-w-md md:max-h-[calc(100vh-2rem)] md:rounded-lg md:shadow-2xl md:border-4 md:border-warmgray-200 md:pointer-events-auto bg-white shadow-2xl border-t-4 border-warmgray-200"
                role="dialog"
                aria-modal="true"
                aria-labelledby="cart-title"
              >
                {/* Cart header - never scrolls */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-warmgray-200 flex-shrink-0">
                  <h2 id="cart-title" className="text-xl font-semibold">{t('cart.shoppingCart')}</h2>
                  <button onClick={() => setIsOpen(false)} aria-label="Close cart">
                    <svg className="w-5 h-5 text-warmgray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                    </svg>
                  </button>
                </div>

                {/* Scrollable items area - flex-1 fills remaining space */}
                <div
                  data-scrollable
                ...
                >
                  {items.length === 0 ? (
                    <p className="text-center text-warmgray-400 py-8">{t('cart.empty')}</p>
                  ) : (
                ...
                <div className="px-6 py-4 border-t border-warmgray-200 flex-shrink-0 bg-cream-50/50">
